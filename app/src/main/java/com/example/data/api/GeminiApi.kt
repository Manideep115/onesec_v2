...
object GeminiService {
    ...
    fun isApiKeyConfigured(): Boolean {
        ...
        return !key.isNullOrEmpty() && key != "MY_GEMINI_API_KEY" && key != "placeholder_api_key"
    }
    ...
    suspend fun generateReflectionPrompt(appName: String, mode: String): String {
        ...
        val request = GenerateContentRequest(
            ...
            generationConfig = GenerationConfig(temperature = 0.7f, maxOutputTokens = 100)
        )
        ...
        return try {
            ...
            Log.e(TAG, "Gemini call failed, falling back to offline content: ${e.message}")
            getOfflinePrompt(appName, mode)
        }
    }
    ...
    suspend fun generateCoachingSession(
        ...
        val request = GenerateContentRequest(
            contents = listOf(Content(parts = listOf(Part(text = prompt)))),
            systemInstruction = Content(parts = listOf(Part(text = systemPrompt))),
            generationConfig = GenerationConfig(temperature = 0.8f, maxOutputTokens = 350)
        )
        ...
        return try {
            ...
            Log.e(TAG, "Gemini call failed for coaching, falling back to offline content: ${e.message}")
            getOfflineCoaching(totalTriggers, avoidedTriggers, savedTimeMinutes, streakDays, topApps)
        }
    }
